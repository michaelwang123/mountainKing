...
//   - DECIMAL, NUMERIC → String (preserve precision)
//   - DATETIME, DATE, TIMESTAMP → DateTime
//   - JSON → JSON
//   - Unsupported types → String (with warning log)
func (m *TypeMapper) MapType(sqlType string) GraphQLType {
	...
	// Strip parenthesized parameters, e.g. VARCHAR(255) → VARCHAR, DECIMAL(10,2) → DECIMAL.
