...
// SQLQueryBuilder converts GraphQL query parameters to parameterized SQL for ClickHouse.
...
func (b *SQLQueryBuilder) Build(req datasource.QueryRequest, table string) (string, []any, error) {
	...
	limitClause := b.buildLimitClause(req.Pagination)
...
// maxINValues is the maximum number of values allowed in a single IN/NOT IN clause
// to prevent query bombing with extremely large parameter lists.
const maxINValues = 10000
...
func buildINClause(col string, value any, negate bool) (string, []any, error) {
	...
	if len(values) > maxINValues {
		return "", nil, apierrors.ValidationError(apierrors.ErrValidationInvalidField,
			fmt.Sprintf("IN/NOT IN operator supports at most %d values, got %d", maxINValues, len(values)))
	}
...
// maxQueryLimit is the upper bound for LIMIT to prevent accidental resource exhaustion.
const maxQueryLimit = 100000

// buildLimitClause builds LIMIT/OFFSET from pagination params.
// ClickHouse does not support parameterized LIMIT/OFFSET (? placeholders),
// so integer values are inlined directly — safe because they are int types.
// Negative values are clamped to 0; limit is capped at maxQueryLimit.
func (b *SQLQueryBuilder) buildLimitClause(p *datasource.PaginationParams) string {
	if p == nil {
		return ""
	}
	...
	// ClickHouse does not support parameterized LIMIT/OFFSET.
	// Inline the integer values directly — safe because they are int types.
	// Clamp negative values and cap limit at maxQueryLimit.
	if limit != nil {
		v := *limit
		if v < 0 {
			v = 0
