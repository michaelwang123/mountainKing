...
//  1. CLOSED â†?OPEN: consecutive failures >= FailureThreshold
//  2. OPEN â†?HALF_OPEN: after OpenDuration elapses
//  3. HALF_OPEN â†?CLOSED: consecutive successes >= SuccessThreshold
//  4. HALF_OPEN â†?OPEN: any failure
//
// Feature: graphql-multi-datasource-api, Property 74: ç†”æ–­å™¨çŠ¶æ€è½¬æ?
// **Validates: Design - ç†”æ–­å™¨å¼¹æ€§è®¾è®?*
func TestProperty74_CircuitBreakerStateTransitions(t *testing.T) {
	rapid.Check(t, func(t *rapid.T) {
		...
		// --- Transition 4: HALF_OPEN â†?OPEN (any failure) ---
		cb.RecordFailure()
		if cb.State() != CircuitOpen {
			t.Fatalf("expected OPEN after failure in HALF_OPEN, got %s", cb.State())
		}

		// --- Transition 3: HALF_OPEN â†?CLOSED ---
...
//
// Feature: graphql-multi-datasource-api, Property 75: ç†”æ–­å™?OPEN çŠ¶æ€å¿«é€Ÿå¤±è´?
// **Validates: Design - ç†”æ–­å™¨å¼¹æ€§è®¾è®?*
