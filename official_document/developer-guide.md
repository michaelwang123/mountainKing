...
├── internal/
�?  ├── adapter/
�?  �?  ├── prometheus/      Prometheus 适配器、查询构建器、类型映射、输入校�?
�?  �?  └── starrocks/       StarRocks 适配器、查询构建器、类型映射、白名单
�?  ├── audit/               审计日志
�?  ├── cache/               缓存接口、内�?Redis 后端、Cache Layer、Key 生成、查询规范化
...
�?  ├── ratelimit/           限流器接口、本�?分布式实现、降级包装器
�?  ├── redis/               共享 Redis 客户�?
�?  ├── sanitize/            敏感信息脱敏
�?  └── server/              HTTP 服务器、路由、优雅关闭、批量查�?
├── pkg/
�?  └── retry/               通用重试逻辑、错误分类器
├── deploy/
�?  ├── Dockerfile           多阶段构�?
...
Schema 文件位于 `internal/graphql/schema/*.graphql`，使�?gqlgen 生成代码�?
...
使用 `pgregory.net/rapid` 库，每个属性测试最少运�?100 次迭代�?

属性测试覆盖的关键领域�?
- 配置校验（拒绝无效值、环境变量覆盖）
- SQL/PromQL 查询构建（参数化、注入防护）
- 类型映射（StarRocks/Prometheus�?
- 缓存行为（Key 确定性、穿�?雪崩/击穿防护�?
...
详见 [数据源适配�?�?扩展新数据源](datasource-adapters.md#扩展新数据源)�?

### 添加新中间件

1. �?`internal/middleware/` 创建中间件文�?
2. 实现 `func(http.Handler) http.Handler` 签名
3. �?`cmd/server/main.go` 的中间件链中注册
4. 编写单元测试和属性测�?
...
| 数据�?| `DATASOURCE_` | `DATASOURCE_TIMEOUT`, `DATASOURCE_UNAVAILABLE` |
