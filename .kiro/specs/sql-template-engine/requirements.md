...
**架构集成方式：** Template_Engine 作为独立组件，直接持有 `*starrocks.Adapter` 引用（非 `DataSource` 接口），通过 Adapter 新增的 `ExecuteRaw` 方法执行渲染后的 SQL，复用现有连接池。`DataSource` 接口不做修改，Prometheus 适配器不受影响。
...
- **Template_Engine**: API_Service 中负责加载、解析和渲染 SQL 模板的组件，直接持有 `*starrocks.Adapter` 引用
...
- **ExecuteRaw**: StarRocks_Adapter 新增的方法，接受原始 SQL 和参数，复用现有连接池执行查询，不经过 SQLQueryBuilder 和白名单校验
...
6. THE Template_Engine SHALL 注册自定义模板函数 `join`（将字符串切片连接为逗号分隔的字符串）和 `quote`（调用 `safeString`（需求 6.1）进行 SQL 转义后，再用单引号包裹返回，即 `'escaped_value'`）。注意：`safeString` 只做转义不加引号，`quote` = 转义 + 包裹引号
7. THE Template_Engine SHALL 支持模板继承（`{{template "name" .}}`）和模板片段（`{{define "name"}}...{{end}}`），允许多个模板共享公共 SQL 片段（如通用的 CTE 定义）
8. THE Rendered_SQL SHALL 为有效的 SQL 语句，THE Template_Engine SHALL 在渲染完成后验证 Rendered_SQL 不为空字符串
...
5. THE `templateList` 查询 SHALL 返回 `[TemplateInfo!]!` 类型，`TemplateInfo` 包含 `name`（String!）、`description`（String!）和 `parameters`（[TemplateParameterInfo!]!）字段，`TemplateParameterInfo` 包含 `name`（String!）、`type`（String!）、`required`（Boolean!）和 `defaultValue`（String）字段
...
5. WHEN Client 请求 `totalCount` 字段时，THE Template_Engine SHALL 执行一条额外的 `SELECT COUNT(*) FROM (Rendered_SQL) AS t` 查询以返回总记录数
...
1. THE StarRocks_Adapter SHALL 提供 `ExecuteRaw(ctx context.Context, query string, args ...interface{}) (*QueryResult, error)` 方法，该方法复用现有的 `*sql.DB` 连接池执行任意 SQL 语句并通过现有的 `scanRows` 函数转换结果。此方法不经过 `SQLQueryBuilder` 和白名单校验，仅供 Template_Engine 内部调用。`DataSource` 接口不做修改
...
5. THE Template_Engine SHALL 遵守现有的查询超时配置（`options.query_timeout`），WHEN 模板查询执行时间超过配置的超时时间时，THE Template_Engine SHALL 取消查询并返回 DATASOURCE_TIMEOUT 错误
...
1. THE Template_Engine SHALL 提供 `safeString` 自定义模板函数，该函数对字符串参数执行 SQL 转义处理（转义单引号 `'` 为 `''`，移除 NULL 字节 `\0`），仅做转义不加引号
...
4. THE Template_Engine SHALL 提供 `safeIdentifier` 自定义模板函数，该函数验证参数仅包含合法 SQL 标识符字符（`[a-zA-Z0-9_.]`）。遇到点号时 SHALL 按 `.` 拆分，分别校验并用反引号包裹（如 `a.b` → `` `a`.`b` ``，`abc` → `` `abc` ``）。IF 参数包含非法字符，THEN 返回模板渲染错误
5. THE Template_Engine SHALL 提供 `safeInList` 自定义模板函数，该函数接受字符串切片参数，对每个元素执行 SQL 转义后生成 `'val1','val2','val3'` 格式的 IN 子句值列表
6. THE Template_Engine SHALL 在模板渲染完成后对 Rendered_SQL 执行基础安全检查：使用简单词法扫描器（状态机追踪单引号边界，处理转义引号 `''`）检测未在字符串字面量内的分号 `;`。扫描器同时 SHALL 检测未在字符串字面量内的 SQL 单行注释（`--`）和块注释（`/* ... */`），IF 检测到注释，THEN 将其从 Rendered_SQL 中移除后再执行后续检查。扫描器实现为线性扫描（O(n)），不需要完整的 SQL 解析器。IF 检测到多条 SQL 语句，THEN 返回 VALIDATION_UNSAFE_SQL 错误
7. THE Template_Engine SHALL 在日志中记录每次模板渲染生成的 Rendered_SQL（脱敏后），用于安全审计
...
9. THE Template_Engine SHALL 支持在 Parameter_Schema 中定义 `string` 类型参数的正则表达式约束（`pattern`），IF 参数值不匹配指定的正则表达式，THEN 返回 VALIDATION_INVALID_PARAMETER_VALUE 错误。正则表达式使用 Go `regexp` 包语法，用于日期格式（如 `^\d{4}-\d{2}-\d{2}$`）、编码格式等常见校验场景
...
2. THE Cache_Layer SHALL 基于模板名称、排序后的参数值和分页参数的组合生成缓存 key，格式为 `cache:template:{template_name}:{xxhash64(sorted_params + first + offset + orderBy)}`，不使用 Rendered_SQL 作为 key 输入（避免模板空白差异导致缓存未命中）
...
6. THE Template_Engine SHALL 同时支持 fsnotify 文件监听自动触发模板重新加载（复用现有 HotReloader 的目录监听模式，监听 `sql_templates.base_dir` 目录变化，500ms 防抖），与 `reloadTemplates` Mutation 手动触发并行工作。自动重新加载的结果通过日志记录（成功/失败模板列表）
7. WHEN 模板重新加载成功后，THE Template_Engine SHALL 自动清除已变更模板的缓存条目（调用 `Cache.DeleteByPrefix("cache:template:{template_name}")`），确保缓存一致性
...
9. THE Template_Engine SHALL 对 `reloadTemplates` 操作使用互斥锁（`sync.Mutex`），防止并发触发多次重新加载导致竞态条件
...
WITH base_events AS (
  ...
  FROM vehicle_events
  WHERE eerid = '{{.Params.eerid | safeString}}'
...
| 函数 | 用途 | 示例 | 输出 |
|------|------|------|------|
| `safeString` | SQL 字符串转义（不加引号） | `{{.Params.name \| safeString}}` | `O''Brien` |
...
| 最大并发模板查询 | ≥ 50 | 受 StarRocks 连接池大小限制（默认 pool_size=20，含单表查询共享） |
...
| # | 威胁 | 攻击向量 | 防御措施 | 对应需求 |
|---|------|---------|---------|---------|
| T1 | SQL 注入 - 字符串参数 | 参数值包含 `'; DROP TABLE --` | `safeString` 转义单引号 + 移除 NULL 字节 | 需求 6.1 |
...
| # | 属性 | 描述 |
|---|------|------|
| P6 | 渲染结果非空 | ∀ 成功渲染的 SQL s：len(s) > 0 |
| P7 | 渲染结果长度限制 | ∀ 成功渲染的 SQL s：len(s) ≤ max_rendered_sql_length |
| P8 | 渲染超时保护 | ∀ 渲染操作 r：r.duration ≤ render_timeout ∨ r.error = INTERNAL_TEMPLATE_RENDER_ERROR |
| P9 | 不存在模板返回错误 | ∀ 模板名称 n ∉ Registry：query(n) 返回 VALIDATION_TEMPLATE_NOT_FOUND |
| P10 | 渲染确定性 | ∀ 模板 t, 参数 p：render(t, p) 的结果在多次调用间一致 |

### GraphQL 集成（需求 3）

| # | 属性 | 描述 |
|---|------|------|
| P11 | templateList 完整性 | templateList 返回的模板集合 = Registry 中所有已注册模板 |
| P12 | templateList 参数一致性 | ∀ 模板 t：templateList 中 t 的 parameters = 配置文件中 t 的 parameters |

### 分页与字段选择（需求 4）

| # | 属性 | 描述 |
|---|------|------|
| P13 | LIMIT 参数化 | ∀ 分页查询：生成的 SQL 中 LIMIT 值通过 `?` 占位符传递 |
| P14 | OFFSET 参数化 | ∀ 分页查询：生成的 SQL 中 OFFSET 值通过 `?` 占位符传递 |
| P15 | 默认 LIMIT 强制 | ∀ 未指定 first 的查询：实际 LIMIT = graphql.max_result_rows |
| P16 | 字段选择安全性 | ∀ fields 中的字段 f：f 通过 safeIdentifier 校验 |
| P17 | OrderBy 字段安全性 | ∀ orderBy 中的字段 f：f 通过 safeIdentifier 校验 |
| P18 | totalCount 独立性 | ∀ 请求 totalCount 的查询：totalCount 值 = COUNT(*) 结果，不受 LIMIT 影响 |

### 查询执行（需求 5）

| # | 属性 | 描述 |
|---|------|------|
| P19 | 结果截断 + 警告 | ∀ 结果行数 > max_result_rows：返回行数 = max_result_rows ∧ extensions.warnings 非空 |
| P20 | 查询超时保护 | ∀ 查询 q：q.duration > query_timeout ⟹ q.error = DATASOURCE_TIMEOUT |
| P21 | 权限检查 | ∀ 无 query 权限的请求：返回 AUTH_INSUFFICIENT_PERMISSION |

### SQL 注入防护（需求 6）

| # | 属性 | 描述 |
|---|------|------|
| P22 | safeString 转义正确性 | ∀ 字符串 s：safeString(s) 不包含未转义的单引号 ∧ 不包含 NULL 字节 |
| P23 | safeInt 类型安全 | ∀ 输入 v：safeInt(v) 成功 ⟹ v 可解析为 int64 |
| P24 | safeFloat 类型安全 | ∀ 输入 v：safeFloat(v) 成功 ⟹ v 可解析为 float64 |
| P25 | safeIdentifier 字符安全 | ∀ 输入 v：safeIdentifier(v) 成功 ⟹ v 仅包含 `[a-zA-Z0-9_.]` |
| P26 | safeIdentifier 反引号包裹 | ∀ 输入 v = "a.b"：safeIdentifier(v) = `` `a`.`b` `` |
| P27 | safeInList 元素独立转义 | ∀ 字符串数组 arr：safeInList(arr) 中每个元素独立经过 safeString 转义 |
| P28 | safeLike 通配符转义 | ∀ 字符串 s 含 `%` 或 `_`：safeLike(s) 中这些字符被转义 |
| P29 | 多语句检测 | ∀ 含字符串外分号的 SQL：安全检查返回 VALIDATION_UNSAFE_SQL |
| P30 | SQL 注释检测 | ∀ 含字符串外 `--` 或 `/*` 的 SQL：注释被移除后再执行检查 |

### 参数校验（需求 7）

| # | 属性 | 描述 |
|---|------|------|
| P31 | 必填参数检查 | ∀ required=true 的参数 p 未提供：返回 VALIDATION_MISSING_PARAMETER |
| P32 | 类型匹配检查 | ∀ 参数 p 类型不匹配 Schema：返回 VALIDATION_INVALID_PARAMETER_TYPE |
| P33 | 默认值填充 | ∀ 可选参数 p 未提供且有 default：渲染时 p = default |
| P34 | 枚举约束 | ∀ 参数 p 有 enum 约束且 p.value ∉ enum：返回 VALIDATION_INVALID_PARAMETER_VALUE |
| P35 | 字符串长度约束 | ∀ string 参数 p：len(p.value) > max_length ⟹ 返回错误 |
| P36 | 数组大小约束 | ∀ string[] 参数 p：len(p.value) > max_items ⟹ 返回错误 |
| P37 | 正则约束 | ∀ string 参数 p 有 pattern 约束：p.value 不匹配 pattern ⟹ 返回错误 |

### 缓存集成（需求 8）

| # | 属性 | 描述 |
|---|------|------|
| P38 | 缓存 key 确定性 | ∀ 相同的 (templateName, params, pagination)：生成相同的缓存 key |
| P39 | 缓存 key 区分性 | ∀ 不同的 (templateName, params, pagination)：生成不同的缓存 key |
| P40 | 模板级 TTL 覆盖 | ∀ 配置了 cache_ttl 的模板 t：缓存 TTL = t.cache_ttl |
| P41 | 缓存禁用 | ∀ cache_enabled=false 的模板 t：查询 t 不写入缓存 |
| P42 | 客户端缓存绕过 | ∀ extensions.cache=false 的请求：不读取缓存 |
| P43 | totalCount 独立缓存 | ∀ 请求 totalCount 的查询：count 缓存 key ≠ data 缓存 key |

### 可观测性（需求 9）

| # | 属性 | 描述 |
|---|------|------|
| P44 | 查询延迟指标记录 | ∀ 模板查询：graphql_template_query_duration_seconds 被记录 |
| P45 | 查询计数指标记录 | ∀ 模板查询：graphql_template_queries_total 递增 |
| P46 | 渲染延迟指标记录 | ∀ 模板渲染：graphql_template_render_duration_seconds 被记录 |
| P47 | Tracing Span 创建 | ∀ 模板查询：创建 `Template Query {name}` 子 Span |
| P48 | 审计日志记录 | ∀ 模板查询：审计日志包含 TemplateName 字段 |

### 热加载（需求 10）

| # | 属性 | 描述 |
|---|------|------|
| P49 | 热加载原子性 | ∀ 热加载期间的查询：使用旧版本或新版本模板，不会看到中间状态 |
| P50 | 错误隔离 | ∀ 热加载中语法错误的模板 t：t 保留旧版本，其他模板正常更新 |
| P51 | 缓存清除 | ∀ 热加载中变更的模板 t：t 的缓存条目被清除 |
| P52 | 并发安全 | ∀ 并发的 reloadTemplates 调用：互斥锁保证串行执行 |
| P53 | 权限检查 | ∀ 无 mutation 权限的 reloadTemplates 调用：返回 AUTH_INSUFFICIENT_PERMISSION |
