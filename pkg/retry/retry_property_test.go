...
func TestProperty36_RetryDistinguishesTransientAndBusiness(t *testing.T) {
	rapid.Check(t, func(t *rapid.T) {
		...
		// Test 2: Business errors should NOT be retried →exactly 1 call
		businessCalls := 0
		Do(context.Background(), cfg, func(ctx context.Context) (int, error) {
			businessCalls++
		...
		// Total expected wait →sum(2^i for i in 0..maxRetries-1) ms
		start := time.Now()
		Do(context.Background(), cfg, func(ctx context.Context) (int, error) {
			return 0, io.EOF
		})
		...
		// Allow some tolerance →elapsed should be at least 50% of expected
